...
class FlappyAgent:
    def __init__(
        ...
        self.target_net.eval()  # Set to evaluation mode (no gradients needed)
    ...
    def select_action(self, state: np.ndarray, epsilon: float) -> int:
        ...
        if random.random() < epsilon:
            # Crucial Fix: Bias randomness towards Waiting (0)
            # 90% chance to Wait, 10% chance to Flap.
            # This allows the bird to fall and find gaps.
    ...
    def recall(self, batch_size: int):
        ...
        return (
            ...
            np.array(next_state),
            np.array(done, dtype=np.uint8),  # 0 or 1
        )
    ...
    def learn(self, batch_size: int = 32) -> float:
        ...
        # 4. Current Q (Prediction)
        # gather(1, actions) extracts the Q-value of the specific action we took
        current_q = self.policy_net(states_t).gather(1, actions_t)

        # 5. Target Q (Ground Truth)
        with torch.no_grad():
            # Use Target Net to predict next best action value
            next_q = self.target_net(next_states_t).max(1)[0].unsqueeze(1)
            # Bellman Equation: R + gamma * MaxQ * (1 - done)
            expected_q = rewards_t + (self.gamma * next_q * (1 - dones_t))

        # 6. Loss Calculation (SmoothL1 is more stable than MSE)
        loss = nn.SmoothL1Loss()(current_q, expected_q)

        # 7. Backpropagation
        self.optimizer.zero_grad()
        loss.backward()
        # Gradient Clipping (prevents exploding gradients, very helpful)
